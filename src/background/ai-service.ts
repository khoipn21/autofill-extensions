...
/**
 * Analyze form using both DOM data and screenshot
 * Includes retry logic for failed JSON parsing and 429 fallback
 * @param formAnalysis Form data to analyze
 * @param maxRetries Maximum retry attempts
 * @param onStream Optional callback for streaming debug output
 */
export async function analyzeFormWithAI(
  ...
  const { apiKey, apiKeys, primaryApiKeyId } = settings;
  ...
  try {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        // Get custom prompt template if set
        ...
        const supportsVision = modelSupportsVision(settings.model);
        const screenshotToUse = supportsVision ? formAnalysis.screenshot : undefined;

        if (!supportsVision && formAnalysis.screenshot) {
          logger.log(`Model ${settings.model} does not support vision, skipping screenshot`);
        }

        const messages = buildMessages(prompt, screenshotToUse);

        logger.log(`Attempt ${attempt}/${maxRetries}: Using model ${settings.model}`);

        // Use streaming if debug mode is enabled and callback is provided
        const useStreaming = settings.debugMode && onStream;
